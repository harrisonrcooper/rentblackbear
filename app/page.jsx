      ...
      {step===FORM&&<div style={{animation:formShake?"shake .4s ease":"fadeUp .3s"}}>
        ...
        <div className="sform">
          <div className="sform-row">
            <div style={{flex:1}}><input className="sinp" placeholder="First Name *" style={fldStyle("firstName")} value={form.firstName} onChange={e=>setForm({...form,firstName:e.target.value.replace(/\b\w/g,c=>c.toUpperCase())})} onBlur={()=>setTouched({...touched,firstName:true})}/>{errMsg("firstName")}</div><div style={{flex:1}}><input className="sinp" placeholder="Last Name *" style={fldStyle("lastName")} value={form.lastName} onChange={e=>setForm({...form,lastName:e.target.value.replace(/\b\w/g,c=>c.toUpperCase())})} onBlur={()=>setTouched({...touched,lastName:true})}/>{errMsg("lastName")}</div>
