...
export default function LeasesTab({
  ...
  const expiringLeases = (leases || []).filter(l => {
    ...
    const leaseEnd = new Date(l.leaseEnd + "T00:00:00");
    const daysLeft = Math.ceil((leaseEnd - TODAY) / 86400000);
    return daysLeft > 0 && daysLeft <= 90;
  }).map(l => {
    const leaseEnd = new Date(l.leaseEnd + "T00:00:00");
    const daysLeft = Math.ceil((leaseEnd - TODAY) / 86400000);
    return { ...l, _daysLeft: daysLeft };
  }).sort((a, b) => a._daysLeft - b._daysLeft);

  const openRenewalModal = (lease) => {
    const newStart = new Date(lease.leaseEnd + "T00:00:00");
    newStart.setDate(newStart.getDate() + 1);
    const newEnd = new Date(newStart);
    newEnd.setFullYear(newEnd.getFullYear() + 1);
    const m2mIncrease = settings.m2mIncrease || 0;
    const suggestedRent = (lease.rent || 0) + m2mIncrease;
    setRenewalModal({
      lease,
      newStart: newStart.toISOString().split("T")[0],
      newEnd: newEnd.toISOString().split("T")[0],
      newRent: lease.rent || 0,
      suggestedRent,
      applyIncrease: false,
    });
  };
  ...
  const createRenewalDraft = async () => {
    ...
    const { lease, newStart, newEnd, newRent, applyIncrease, suggestedRent } = renewalModal;
    const finalRent = applyIncrease ? suggestedRent : newRent;
    ...
    const renewalLease = {
      ...
      leaseEnd: newEnd,
      leaseType: "fixed",
      doorCode: lease.doorCode || "",
      parking: lease.parking || "",
      parkingChoice: lease.parkingChoice || null,
      utilitiesMode: lease.utilitiesMode || "",
      utilitiesClause: lease.utilitiesClause || "",
      landlordName: lease.landlordName || settings.pmName || "",
      company: lease.company || settings.companyName || "",
      landlordEmail: lease.landlordEmail || settings.email || "",
      agreementDate: TODAY.toISOString().split("T")[0],
      sections: lease.sections || [],
      addenda: lease.addenda || [],
      notes: "Renewal of lease ending " + fmtD(lease.leaseEnd),
      renewalOf: lease.id,
    ...
    const row = {
      ...
      variable_data: renewalLease,
      status: "draft",
      updated_at: new Date().toISOString(),
    };
    try {
      await supa("lease_instances", { method: "POST", prefer: "resolution=merge-duplicates", body: JSON.stringify(row) });
    } catch (e) { console.error("Renewal upsert error:", e); }
    setLeases(p => [...p, renewalLease]);
    setRenewalModal(null);
    showAlert({ title: "Renewal Draft Created", body: "Renewal draft created for " + (renewalLease.tenantName || "tenant") + ". Open it in Leases to review and send." });
  };
  ...
  return (<>
    ...
    {/* ── Lease Renewals — expiring executed leases ── */}
    {leaseSubTab === "leases" && expiringLeases.length > 0 && (
      <div style={{ marginTop: 24 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#9a7422" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="1 4 1 10 7 10" /><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10" /></svg>
          <div style={{ fontSize: 15, fontWeight: 800 }}>Upcoming Renewals</div>
          <span style={{ fontSize: 11, fontWeight: 600, color: "#9a7422", background: "rgba(212,168,83,.1)", padding: "2px 10px", borderRadius: 100 }}>{expiringLeases.length}</span>
        </div>
        <div style={{ fontSize: 11, color: "#6b5e52", marginBottom: 12 }}>Executed leases expiring within 90 days. Create a renewal draft to offer a new term.</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {expiringLeases.map(lease => {
            const urgency = lease._daysLeft <= 7 ? { bg: "rgba(196,92,74,.08)", border: "rgba(196,92,74,.2)", color: "#c45c4a", label: "Urgent" }
              : lease._daysLeft <= 30 ? { bg: "rgba(212,168,83,.08)", border: "rgba(212,168,83,.2)", color: "#9a7422", label: "Soon" }
              : { bg: "rgba(74,124,89,.06)", border: "rgba(74,124,89,.15)", color: "#2d6a3f", label: "Upcoming" };
            return (
              <div key={lease.id} className="card" style={{ border: "1px solid " + urgency.border }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1 }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 700 }}>{lease.tenantName}</div>
                      <div style={{ fontSize: 11, color: "#6b5e52" }}>{lease.property} {lease.room ? " -- " + lease.room : ""}</div>
                    </div>
                    <div style={{ marginLeft: "auto", textAlign: "right", marginRight: 12 }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: urgency.color }}>{lease._daysLeft} days left</div>
                      <div style={{ fontSize: 10, color: "#6b5e52" }}>Expires {fmtD(lease.leaseEnd)} | {fmtS(lease.rent || 0)}/mo</div>
                    </div>
                    <span style={{ fontSize: 10, fontWeight: 700, padding: "3px 10px", borderRadius: 100, background: urgency.bg, color: urgency.color, whiteSpace: "nowrap" }}>{urgency.label}</span>
                  </div>
                  <button className="btn btn-gold btn-sm" style={{ marginLeft: 12, whiteSpace: "nowrap" }} onClick={() => openRenewalModal(lease)}>Renew</button>
                </div>
              </div>
            );
          })}
    ...
    {/* ── Renewal Modal ── */}
    {renewalModal && <div className="mbg" onClick={() => setRenewalModal(null)}>
      <div className="mbox" onClick={e => e.stopPropagation()} style={{ maxWidth: 500 }}>
        <h2>Renew Lease</h2>
        <p style={{ fontSize: 12, color: "#5c4a3a", marginBottom: 16 }}>Create a renewal draft for this tenant. You can review and edit it before signing.</p>

        <div style={{ background: "rgba(74,124,89,.06)", borderRadius: 10, padding: 12, marginBottom: 14 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#2d6a3f", marginBottom: 8 }}>CURRENT LEASE</div>
          <div className="fr" style={{ gap: 8 }}>
            <div style={{ flex: 1, fontSize: 12 }}><span style={{ color: "#6b5e52" }}>Tenant:</span> <strong>{renewalModal.lease.tenantName}</strong></div>
            <div style={{ flex: 1, fontSize: 12 }}><span style={{ color: "#6b5e52" }}>Property:</span> <strong>{renewalModal.lease.property} {renewalModal.lease.room ? " -- " + renewalModal.lease.room : ""}</strong></div>
          </div>
          <div className="fr" style={{ gap: 8, marginTop: 6 }}>
            <div style={{ flex: 1, fontSize: 12 }}><span style={{ color: "#6b5e52" }}>Current Rent:</span> <strong>{fmtS(renewalModal.lease.rent || 0)}/mo</strong></div>
            <div style={{ flex: 1, fontSize: 12 }}><span style={{ color: "#6b5e52" }}>Expires:</span> <strong>{fmtD(renewalModal.lease.leaseEnd)}</strong></div>
          </div>
        </div>

        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
          <div className="fld">
            <label>New Lease Start</label>
            <input type="date" value={renewalModal.newStart} onChange={e => setRenewalModal(p => ({ ...p, newStart: e.target.value }))} />
          </div>
          <div className="fld">
            <label>New Lease End</label>
            <input type="date" value={renewalModal.newEnd} onChange={e => setRenewalModal(p => ({ ...p, newEnd: e.target.value }))} />
          </div>
          <div className="fld">
            <label>Monthly Rent</label>
            <input type="number" value={renewalModal.applyIncrease ? renewalModal.suggestedRent : renewalModal.newRent}
              onChange={e => setRenewalModal(p => ({ ...p, newRent: Number(e.target.value), applyIncrease: false }))} />
          </div>
          {(settings.m2mIncrease || 0) > 0 && (
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, cursor: "pointer", padding: "8px 12px", background: "rgba(212,168,83,.06)", borderRadius: 8, border: "1px solid rgba(212,168,83,.15)" }}>
              <input type="checkbox" checked={!!renewalModal.applyIncrease}
                onChange={e => setRenewalModal(p => ({ ...p, applyIncrease: e.target.checked, newRent: e.target.checked ? p.suggestedRent : (p.lease.rent || 0) }))}
                style={{ width: 14, height: 14 }} />
              Apply {fmtS(settings.m2mIncrease)}/mo increase ({fmtS(renewalModal.suggestedRent)}/mo total)
            </label>
          )}
        </div>

        <div className="mft">
          <button className="btn btn-out" onClick={() => setRenewalModal(null)}>Cancel</button>
          <button className="btn btn-gold" onClick={createRenewalDraft}>Create Renewal Draft</button>
        </div>
      </div>
    </div>}
