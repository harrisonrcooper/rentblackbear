...
const DEF_LEASE_SECTIONS=[
          ...
          {id:"s2",title:"Payments",requiresInitials:true,active:true,content:"<p>Rent and other charges are to be paid at such place or method designated by PROPERTY MANAGER. All payments are to be made via the resident portal at {{PORTAL_URL}} or any other method approved by PROPERTY MANAGER.</p><p>PROPERTY MANAGER acknowledges receipt of the First Month's Rent and a Security Deposit of <strong>${{SECURITY_DEPOSIT}}</strong>. All payments are to be made payable to <strong>{{LANDLORD_NAME}}</strong>.</p>"},
...
.bot-fab{display:none;position:fixed;left:50%;bottom:calc(70px + env(safe-area-inset-bottom));transform:translateX(-50%);width:56px;height:56px;border-radius:50%;border:none;cursor:pointer;z-index:101;align-items:center;justify-content:center;box-shadow:0 6px 24px rgba(0,0,0,.35),0 2px 6px rgba(0,0,0,.2);transition:transform .12s ease,box-shadow .12s ease;color:#fff;font-family:inherit}
.bot-fab:active{transform:translateX(-50%) scale(.94);box-shadow:0 3px 12px rgba(0,0,0,.3)}
...
export default function Page(){
  ...
  return(<div style={{fontFamily:_font}}><style>{S}</style><style>{adminDynCSS(_acc,_rgb)}</style><div className="app">
    ...
    <div className="mn" style={{zoom:_zoom,left:220*_zoom}}>
      ...
      {/* ═══ DOCUMENTS ═══ */}
      {tab==="documents"&&<DocumentsTab docs={docs} setModal={setModal} />}
      ...
      {/* ═══ NOTIFICATIONS ═══ */}
      {tab==="notifications"&&<NotificationsTab notifs={notifs} setNotifs={setNotifs} unreadCount={m.unreadNotifs} />}
