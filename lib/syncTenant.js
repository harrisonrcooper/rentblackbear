  ...
  try {
    ...
    if (propName) {
      ...
      if (props?.[0]?.id) {
        ...
        const created = await supaUpsert("properties", { pm_id: pmId, name: propName, ...(workspaceId ? { workspace_id: workspaceId } : {}) });
        propId = created?.[0]?.id;
      }
    }
    ...
    if (roomName && propId) {
      ...
      if (rooms?.[0]?.id) {
        ...
        if (units?.[0]?.id) {
          ...
          const createdUnit = await supaUpsert("units", { pm_id: pmId, property_id: propId, name: "Unit A", rental_mode: "byRoom", ...(workspaceId ? { workspace_id: workspaceId } : {}) });
          unitId = createdUnit?.[0]?.id;
        }
        if (unitId) {
          const createdRoom = await supaUpsert("rooms", { pm_id: pmId, unit_id: unitId, property_id: propId, name: roomName, rent: rent || 0, status: "occupied", door_code: doorCode || null, ...(workspaceId ? { workspace_id: workspaceId } : {}) });
