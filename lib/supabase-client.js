...
import { wsKey } from "@/lib/workspace";

const SUPA_URL = process.env.NEXT_PUBLIC_SUPABASE_URL;
const SUPA_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

// ── Workspace isolation ────────────────────────────────────────────
// When set, all loadAppData/saveAppData calls auto-prefix keys with
// the workspace_id. Callers keep using bare keys like "hq-props" and
// the prefix is applied transparently.  null = backwards-compat mode.
let _workspaceId = null;

/**
 * Set the active workspace for all subsequent load/save calls.
 * Call this once after Clerk auth resolves.
 * @param {string|null} id — Clerk userId or workspace id
 */
...
/**
 * Get the current workspace id (for debugging / migration).
 * @returns {string|null}
 */
...
export async function loadAppData(key, fallback) {
  const fullKey = wsKey(_workspaceId, key);
  try {
    const r = await supa("app_data?key=eq." + encodeURIComponent(fullKey) + "&select=value");
...
export async function saveAppData(key, data) {
  ...
  try {
    await supa("app_data", {
      ...
      body: JSON.stringify({ key: fullKey, value: data }),
    });
  } catch (e) {
    console.error("saveAppData error:", fullKey, e);
  }
}
