...
//
// The template is *inspired by* common household + small-portfolio
// property-manager budgets (the same shape as Harrison's 10X.xlsx)
// but every amount starts at $0 and every label is editable. The
// point is to spare the user from staring at an empty page on first
// load — give them the right rows in the right groups, let them fill
// in numbers.
//
// Calling seedBudget on a workspace that already has data is a no-op
// (we don't want to clobber edits).
...
const TEMPLATE_CATEGORIES: ImportCategory[] = [
  // Giving
...
// One blank property + one blank debt give the user something concrete
// to click on instead of a "where do I even start?" empty panel.
...
export async function seedBudget(): Promise<{ ok: boolean; message?: string }> {
  const { userId } = await auth();
  if (!userId) return { ok: false, message: "Not authenticated." };
  const allowedOwner = process.env.BUDGET_OWNER_USER_ID;
  if (allowedOwner && userId !== allowedOwner) {
    return { ok: false, message: "Not authorized." };
  }
  try {
    const existing = await loadBudgetState(userId);
    // No-op if anything is already there — we never overwrite user data.
    ...
    await saveBudgetState(userId, seeded, userId);
    return { ok: true };
  } catch (e: unknown) {
